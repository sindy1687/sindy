...
const LinkageSystem = {
    ...
    stars: {
        ...
        updateDisplay: () => {
            ...
            elements.forEach(el => {
                if (el) el.textContent = currentStars;
            });
        }
    },
    ...
    // 卡片系統
    cards: {
        getOwnedCards: () => JSON.parse(localStorage.getItem("ownedCards") || "{}"),
        setOwnedCards: (cards) => {
            localStorage.setItem("ownedCards", JSON.stringify(cards));
        },
        getShards: () => JSON.parse(localStorage.getItem("cardShards") || "{}"),
        setShards: (shards) => {
        ...
        addShard: (word, count = 1) => {
            const shards = LinkageSystem.cards.getShards();
            shards[word] = (shards[word] || 0) + count;
    ...
    notifications: {
        show: (message, type = 'info', duration = 3500) => {
            ...
            setTimeout(() => {
                notification.style.animation = 'fadeOutUp 0.5s cubic-bezier(0.55, 0.085, 0.68, 0.53) forwards';
                setTimeout(() => {
                    notification.remove();
                }, 500);
            }, duration);
