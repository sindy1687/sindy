...
const SoundSystem = {
  ...
  init() {
    ...
    this.audio.complete = document.getElementById('completeSound') || new Audio('sound/completeSound.mp3');
...
// 碎片獎勵系統
const ShardRewardSystem = {
  // 根據遊戲表現給予碎片獎勵
  giveGameReward(gamePerformance) {
    const { correctCount, totalQuestions, timeBonus, perfectBonus } = gamePerformance;
    const accuracy = correctCount / totalQuestions;
    
    // 基礎獎勵：根據正確率決定
    let baseReward = 0;
    if (accuracy >= 0.9) baseReward = 3; // 90%以上正確率
    else if (accuracy >= 0.8) baseReward = 2; // 80%以上正確率
    else if (accuracy >= 0.6) baseReward = 1; // 60%以上正確率
    
    // 額外獎勵
    const timeBonusReward = timeBonus || 0;
    const perfectBonusReward = perfectBonus || 0;
    
    const totalReward = baseReward + timeBonusReward + perfectBonusReward;
    ...
    if (totalReward > 0) {
      ...
      if (allCards.length > 0) {
        ...
        this.addShard(randomCard.word, totalReward);
        
        return {
          card: randomCard,
          shards: totalReward,
          message: `🎉 遊戲完成！獲得 ${randomCard.zh} 碎片 x${totalReward}`
        };
      }
    }
  ...
  // 添加碎片
  addShard(cardWord, amount = 1) {
    const shards = JSON.parse(localStorage.getItem('cardShards') || '{}');
    const ownedCards = JSON.parse(localStorage.getItem('ownedCards') || '{}');
    
    // 如果已經擁有該卡片，給予星星補償
    if (ownedCards[cardWord]) {
      const currentStars = parseInt(localStorage.getItem('totalStars') || '0');
      const bonusStars = amount * 2; // 每片碎片補償2顆星星
      localStorage.setItem('totalStars', currentStars + bonusStars);
      return {
        type: 'compensation',
        stars: bonusStars,
        message: `已擁有該卡片，獲得 ${bonusStars} 顆星星補償`
      };
    }
    
    // 添加碎片
    shards[cardWord] = (shards[cardWord] || 0) + amount;
    localStorage.setItem('cardShards', JSON.stringify(shards));
    
    // 檢查是否集滿碎片
    const card = (window.allCards || []).find(c => c.word === cardWord);
    if (card) {
      const required = this.getRequiredShards(card.rarity);
      if (shards[cardWord] >= required && !ownedCards[cardWord]) {
        // 修復：只有在未擁有該卡片時才設置為已擁有
        ownedCards[cardWord] = true;
        localStorage.setItem('ownedCards', JSON.stringify(ownedCards));
        
        // 修復：在遊戲獎勵解鎖卡片後，檢查成就
        // 注意：這裡需要一個全局的 checkAchievements 函數
    ...
    return {
      type: 'shard',
      shards: shards[cardWord],
      required: card ? this.getRequiredShards(card.rarity) : 1,
      message: `✨ 獲得 ${card?.zh || cardWord} 碎片！`
    };
  },
...
if (typeof window !== 'undefined') {
  ...
  window.ShardRewardSystem = ShardRewardSystem;
}
